...
from lib.utils.quantize_utils import QConv2d
from lib.utils.binary_utils import BinarizeConv2d
...
class QBasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, cardinality=1, base_width=64,
        ...
        if a_bit_1 == 1:
            ...
            self.conv1 = QConv2d(
            inplanes, first_planes, kernel_size=3, stride=1 if use_aa else stride, padding=first_dilation,
            dilation=first_dilation, bias=False, w_bit=4, a_bit=4)
            self.conv2 = QConv2d(
                first_planes, outplanes, kernel_size=3, padding=dilation, dilation=dilation, bias=False, w_bit=4,
                a_bit=4)
        self.bn1 = norm_layer(first_planes)
        self.act1 = act_layer(inplace=True)
        self.aa = aa_layer(channels=first_planes, stride=stride) if use_aa else None
        ...
        '''self.conv2 = QConv2d(
            first_planes, outplanes, kernel_size=3, padding=dilation, dilation=dilation, bias=False, w_bit=1, a_bit=1)'''
        self.bn2 = norm_layer(outplanes)
...
def cel_max_min(x):
    # print(x.shape)
    x_cpu = x.transpose(0, 1) #.flatten().detach()
    x_max_list = []
    for i in range(64):
        x_max = torch.max(x_cpu[i])
        x_max_list.append(x_max.item())
        # print(x_max)
    # print(x_max_list)
    print(min(x_max_list) / max(x_max_list))
...
def resnet18(pretrained=False, **kwargs):
    ...
    """
    print("use resnet18 binary")
    model_args = dict(block=QBasicBlock, layers=[2, 2, 2, 2], **kwargs)
