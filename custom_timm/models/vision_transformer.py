...
from .layers import PatchEmbed, Mlp, QMlp, DropPath, trunc_normal_, lecun_normal_
from .registry import register_model
from lib.utils.quantize_utils import QLinear, QConv2d
from lib.utils.quantize_utils_activation import QActvation
...
class QAttention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., **kwargs):
        ...
        bitwidth = self.args.bitwidth
        self.num_heads = num_heads
        head_dim = dim // num_heads
        self.scale = qk_scale or head_dim ** -0.5
        #self.qkv = nn.Linear(dim, dim * 3, bias=qkv_bias)
        self.qkv = QLinear(dim, dim * 3, bias=qkv_bias, w_bit=bitwidth, a_bit=bitwidth, half_wave='Q')
        #self.qkv._calibrate = True
        self.qkv_actvation = QActvation(dim * 3, a_bit=8, half_wave='Q')
        self.attn_actvation = QActvation(dim, a_bit=8, half_wave='Q')
        self.attn_drop = nn.Dropout(attn_drop)
        #self.proj = nn.Linear(dim, dim)
        self.proj = QLinear(dim, dim, w_bit=bitwidth, a_bit=bitwidth, half_wave='Q')
        #self.proj._calibrate = True
        self.proj_drop = nn.Dropout(proj_drop)
    ...
    def forward(self, x):
        ...
        # qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        qkv = self.qkv(x)
        qkv = self.qkv_actvation(qkv).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        q, k, v = qkv[0], qkv[1], qkv[2]   # make torchscript happy (cannot use tensor as tuple)

        attn = (q @ k.transpose(-2, -1)) * self.scale
        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)
        attn = self.attn_actvation(attn)
...
class QPatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768, norm_layer=None):
        ...
        self.proj = QConv2d(in_chans, embed_dim, kernel_size=patch_size, stride=patch_size, w_bit=8, a_bit=8, half_wave='Q')
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0., embed_layer=QPatchEmbed, norm_layer=None,
        ...
        self.blocks = nn.Sequential(*[
            QBlock(
                ...
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, act_layer=act_layer,**kwargs)
        ...
        self.head = QLinear(self.num_features, num_classes, w_bit=8, a_bit=8, half_wave='Q') if num_classes > 0 else nn.Identity()
        self.head_dist = None
        if distilled:
