...
from .layers import PatchEmbed, Mlp, QMlp, DropPath, to_2tuple, trunc_normal_
...
default_cfgs = {
    ...
    'fpswin_small_patch4_window7_224': _cfg(
        url='https://github.com/SwinTransformer/storage/releases/download/v1.0.0/swin_small_patch4_window7_224.pth',
    ),
    ...
    'fpswin_tiny_patch4_window7_224': _cfg(
        url='https://github.com/SwinTransformer/storage/releases/download/v1.0.0/swin_tiny_patch4_window7_224.pth',
    ),
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.attent = torch.zeros(1, 3, 197, 197)
...
class QWindowAttention(nn.Module):
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0., **kwargs):
        ...
        #self.qkv = nn.Linear(dim, dim * 3, bias=qkv_bias)
        self.qkv = QLinear(dim, dim * 3, bias=qkv_bias, **kwargs)
        self.attn_drop = nn.Dropout(attn_drop)
        # self.proj = nn.Linear(dim, dim)
        self.proj = QLinear(dim, dim, **kwargs)
        self.proj_drop = nn.Dropout(proj_drop)
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        quantization = kwargs.pop('quant', False)
        if quantization == True:
            self.attn = QWindowAttention(
                dim, window_size=to_2tuple(self.window_size), num_heads=num_heads,
                qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop, **kwargs)
            self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
            self.norm2 = norm_layer(dim)
            mlp_hidden_dim = int(dim * mlp_ratio)
            self.mlp = QMlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop, **kwargs)
        else:
...
def _create_swin_transformer(variant, pretrained=False, default_cfg=None, quantization=False, **kwargs):
...
def swin_base_patch4_window7_224(pretrained=False, **kwargs):
    ...
    return _create_swin_transformer('fpswin_base_patch4_window7_224', pretrained=pretrained, **model_kwargs)
...
def swin_small_patch4_window7_224(pretrained=False, **kwargs):
    ...
    return _create_swin_transformer('swin_small_patch4_window7_224', pretrained=pretrained, quantization=True, **model_kwargs)

@register_model
def fpswin_small_patch4_window7_224(pretrained=False, **kwargs):
    """ Swin-S @ 224x224, trained ImageNet-1k
    """
    model_kwargs = dict(
        patch_size=4, window_size=7, embed_dim=96, depths=(2, 2, 18, 2), num_heads=(3, 6, 12, 24), **kwargs)
    return _create_swin_transformer('fpswin_small_patch4_window7_224', pretrained=pretrained, quantization=False, **model_kwargs)
...
def swin_tiny_patch4_window7_224(pretrained=False, **kwargs):
    ...
    return _create_swin_transformer('swin_tiny_patch4_window7_224', pretrained=pretrained, quantization=True, **model_kwargs)

@register_model
def fpswin_tiny_patch4_window7_224(pretrained=False, **kwargs):
    """ Swin-T @ 224x224, trained ImageNet-1k
    """
    model_kwargs = dict(
        patch_size=4, window_size=7, embed_dim=96, depths=(2, 2, 6, 2), num_heads=(3, 6, 12, 24), **kwargs)
    return _create_swin_transformer('fpswin_tiny_patch4_window7_224', pretrained=pretrained, 
                                    quantization=False, **model_kwargs)
