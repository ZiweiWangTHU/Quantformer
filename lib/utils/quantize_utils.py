...
class QModule(nn.Module):
    def __init__(self, w_bit=8, a_bit=8, half_wave=True, dim=192, **kwargs):
    ...
    def QuantGroup(self, input):
        ...
        for i, branch in enumerate(self.mix_activ_mark1):
            ...
            outs_x.append(x * self.sw[i])
            outs_ori.append(ori_x * self.sw[i])
        activ = sum(outs_x)
    ...
    def _quantize_activation(self, inputs):
        ...
        if self._quantized and self._a_bit > 0:
            if self._calibrate:
                ...
                activation_range2, groups_range = calculate_group_range(activation_range2, mlp2=C, group_n=self.group_n)

                if self.args.search == False and self.group_n > 1:
                    for l in range(self.group_n):
                        if groups_range[l] == 0:
                            groups_range[l] = self.init_range
                    max_indix_list = torch.argmax(self.alpha_activ.data, dim=0)
                    for i in range(C):
                        self.activation_range[i] = groups_range[max_indix_list[i]]
                    # print(self.alpha_activ, self.sw, max_indix_list, self.activation_range)
                else:
                    self.activation_range = torch.tensor(activation_range2.detach().clone(),
            ...
            if self.args.search == True:
                ...
                self.activation_range = self.activation_range.to(inputs.device)
                scaling_factor1 = self.activation_range / (2. ** self._a_bit - 1.)
...
class QLinear(QModule):
    def __init__(self, in_features, out_features, bias=True, w_bit=8, a_bit=8, half_wave=True, dim=192, **kwargs):
...
from numpy import *
def calculate_group_range(x, mlp2, group_n):
    C = mlp2
    range_max = x.max()
    range_min = x.min()
