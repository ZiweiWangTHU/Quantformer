    ...
    def _quantize_activation(self, inputs):
        ...
        if self._quantized and self._a_bit > 0:
            if self._calibrate:
                ...
                self.range1 = torch.zeros(C)
                # sw = F.softmax(self.alpha_activ, dim=0)
                '''max_indix = torch.argmax(self.alpha_activ.data)
                self.range1 = self.groups_range1.data[max_indix]
                print(self.alpha_activ, max_indix)'''
                max_indix_list = torch.argmax(self.alpha_activ.data, dim=0)
                for i in range(C):
                    self.range1[i] = self.groups_range1.data[max_indix_list[i]]
                print(self.alpha_activ, max_indix_list, self.range1)
            ...
            activation_r = torch.tensor(self.range1.clone(), device=inputs.device, requires_grad=True)
            # print(activation_r, activation_r.grad)
            scaling_factor1 = activation_r / (2. ** self._a_bit - 1.)
